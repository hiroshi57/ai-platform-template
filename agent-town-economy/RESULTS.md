...
- モデル: `qwen2.5:0.5b-instruct`（CPU推論、GPU無し）、malformed **0**（全て有効なJSON）
- 実行: `python experiments/llm_vs_heuristic.py --geojson data/lakeside.geojson`
  （生結果は `experiments/results.json`）
...
| **LLM (Qwen0.5B)** | low | 0.43 | **0** | **0** | 0 | 0 | 240 | 150 |
| **LLM (Qwen0.5B)** | high | 0.24 | **0** | **0** | 0 | 0 | 234 | 234 |

### 所見

1. **価格・賃金レバーは実LLMでも一度も使われない**（`set_price`/`set_wage` が全4ランで 0 回）。
   → 論文の中核「クリーンな需要ショックでも賃金・価格に伝播しない（伝播失敗）」は、
   **自己ホストの実LLMでも再現**された。
2. **賃金シェアは観光増で低下**（LLM: 0.43→0.24、ヒューリスティック: 4.61→0.48）。方向は論文
   （0.90→0.20）と一致。
3. Qwen2.5-0.5B は**全240決定で `start_shift` を選ぶ退化的挙動**（malformed 0＝形式は常に正しい）。
   小型モデルゆえ多様な行動は取らないが、**レバー未使用＝伝播失敗という定性結論は覆らなかった**。

### 限界

- 0.5B は極小モデルで挙動が単調。より大きな指示追従モデル（Qwen2.5-7B/14B 等）や、
  「売り切れたら賃上げも選択肢」と促すプロンプトでは結果が変わりうる（＝論文の反証可能性）。
- CPU推論のため1ラン約9〜11分。大型モデル・長horizon・多シードは GPU + vLLM 推奨。
- 本実験は「装置が実LLMで規模実行でき、伝播失敗が実モデルでも観測される」ことの実証であり、
  モデル一般化の主張ではない。
