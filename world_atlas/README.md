...
国連(SDG Global Database・UNDP・UNHCR・UNESCO)・世界銀行・CIA World Factbook・世界の大学(オックスフォード /
フローニンゲン / ヨーテボリ / ウプサラ / ハーバード / イェール / ノートルダム / UCLA)の公開データを集め、**200か国以上を3D地球儀と図鑑ページで比べられる**
...
| モード | 中身 |
|---|---|
| 🌐 地球儀 | 指標で国を色分けし、**値の大きさで国を立体的に持ち上げる3D表示**(3D棒グラフ・平らな地図にも切りかえ可)。**地図帳のような国名**(ズームで段階表示)。年スライダーと ▶ 再生で過去から現在へ(歴史の章は紀元前1万年から)。難民の指標では**出身国→受入国の流れを弧で表示** |
| 📘 図鑑ページ | 国旗・基本情報・日本語の概要(Wikipedia)・CIA Factbook の地理/気候/言語/宗教/政治(歴史の背景は**日本語要約**付き・確認状態を表示)・**主な輸出品目**(ハーバード大 Atlas)・**世界全体との差・順位・S〜D評価・10年の推移・2030年予測・総評**。🖨️ **印刷用ページ**(A4 の1国冊子・PDF 保存可) |
| 🏆 ランキング | 地域で絞り込み、世界全体の値の線付き |
| ⚖️ くらべる | 最大4か国の推移(予測の点線付き)・章ごとのレーダー・指標表 |
| 🧩 分類 | 地域/所得グループ別の分布と中央値・2指標の散布図(相関係数と読み方)・「データが似ている国」 |
| 📜 ビジュアル年表 | 古代〜現代の111件+各国の独立・建国174件(自動)。**歴史上の人物78人をオリジナルのキャラクターで描き**、関係する国が3D地球儀で光り、その国の上にキャラクターが立つ。時代別のキャラクター図鑑。背景に世界人口の推移 |
| 🎯 SDGs | 17目標のタイル(コンパクト表示)と国別スコア・目標ごとの関連指標(**国連の公式 SDG 指標**を含む)・上位/下位の国・スコアを地球儀で表示 |
| 🧠 クイズ | 最新データから10問を自動生成(くらべる/世界一/年表/人物/地理/SDGs)。**問題番号**でクラス全員が同じ問題に。解説と「図鑑で見る」リンク。**ワークシート印刷**(解答は別ページ) |
| 📚 出典 | 全指標の出典・期間・取得状態・注意事項 |

章立て(10章・98指標): 地理・地形 / 人口 / 気候・環境 / 生き物・自然(生物学) / 経済・くらし /
...
UCLA WORLD Policy Analysis Center の法律データ(ゲストブック回答が必要・CC BY-NC 4.0):

```bash
python -m world_atlas.pipeline.build_data --only ucla --skip factbook,wiki,geo,flows,exports
```

ゲストブックの回答内容(名前・メール・所属・役職・国・目的)は `~/sekai-zukan.guestbook.env.txt` に置きます(リポジトリには入れない)。
週次の自動更新では取得しません(法律データの更新は数年ごとのため)。

CIA 原文の日本語要約(AI の下書き → 人が確認して承認):

```bash
python -m world_atlas.pipeline.factbook_ja --status
```

```bash
python -m world_atlas.pipeline.factbook_ja --review JPN --reviewer "山田(社会科)"
```

`--draft-missing 20` で未作成の国の下書きを作れます(`ANTHROPIC_API_KEY` と `ANTHROPIC_MODEL` が必要)。
画面では「AI 下書き(確認前)」「確認済み」「原文が改訂されたため要再確認」を区別して表示します。
...
```bash
node --test world_atlas/site/js/analytics.test.mjs world_atlas/site/js/quiz.test.mjs
```
...
本番(production)への昇格は、CLAUDE.md の規定に従い人/Cursor 側で行ってください。
